...
def ci2ls(ci,zeroindexed=False):
	...
	zero indexing refers to the ci vector; the list is NEVER zeroindexed
	'''
...
(This is true.  The algorithm depends on a random seed. ~~rlaplant)
	...
	for m in xrange(np.max(ci)+1):	#loop over modules
...
(This is true.  The algorithm depends on a random seed ~~rlaplant)
	...
	for m in xrange(np.max(ci)+1):			#loop over initial modules
		Knm0[:,m]=np.sum(W0[:,ci==m],axis=1)
		Knm1[:,m]=np.sum(W1[:,ci==m],axis=1)
			...
			if not (R[a,d] or R[c,b]):
				...
				if (D[a,b]*R[a,b]+D[c,d]*R[c,d]>=D[a,d]*R[a,b]+D[c,b]*R[c,d]):
					...
					if rewire:					#reassign edges
						...
						j.itemset(e1,d)			#reassign edge indices
						j.itemset(e2,b)
						eff+=1
						break
			att+=1
	...
	for it in xrange(iter):
		...
		while att<=max_attempts:
			...
			if np.random.random()>.5:
				i.itemset(e2,d)			#flip edge c-d with 50% probability
				j.itemset(e2,c)			#to explore all potential rewirings
				c=i[e2]; d=j[e2]
...
Output:     R,      randomized network
	...
	for it in xrange(iter):					#while not rewired
		while True:
			...
			if np.random.random()>.5:
				icur.itemset(e2,d)			#flip edge c-d with 50% probability
				jcur.itemset(e2,c)			#to explore all potential rewirings
				c=icur[e2]; d=jcur[e2]
			#rewiring condition
			if not (R[a,d] or R[c,b]):
				R[a,d]=R[a,b]; R[a,b]=0
				...
				R[c,b]=R[c,d]; R[c,d]=0
				R[b,c]=R[d,c]; R[d,c]=0
				jcur.itemset(e1,d)			#reassign ege indices
				jcur.itemset(e2,b)
				break
...
Outputs:    R,          randomized network
	...
	for it in xrange(k):
		...
		if np.size(ii):
			...
			for m in xrange(k):
				if i[m]==d and j[m]==c:
					i.itemset(it,c); j.itemset(m,b)
				elif i[m]==c and j[m]==d:
					j.itemset(it,c); i.itemset(m,b)
