...
def load_sample(thres=1):
	return bct.threshold_proportional(np.load(os.path.join(MAT_DIR, 'sample_data.npy')), thres,
		copy=False)

def load_signed_sample(thres=1):
	return bct.threshold_proportional(np.around(
		np.load(os.path.join(MAT_DIR, 'sample_signed.npy')),8), thres, copy=False)
...
def load_directed_sample(thres=1):
	return bct.threshold_proportional(np.load(os.path.join(MAT_DIR, 'sample_directed.npy')), 
		thres, copy=False)
...
def load_directed_low_modularity_sample(thres=1):
	return bct.threshold_proportional(np.load(os.path.join(MAT_DIR, 'sample_directed_gc.npy')),
		thres, copy=False)
...
def load_sample_group_qball():
	q = np.load(os.path.join(MAT_DIR, 'sample_group_qball.npy'))
	return np.transpose(
		list(map(bct.normalize,(q[:,:,i] for i in range(q.shape[2])))),
		(1,2,0))

def load_sample_group_dsi():
	d = np.load(os.path.join(MAT_DIR, 'sample_group_dsi.npy'))
	return np.transpose(
		list(map(bct.normalize,(d[:,:,i] for i in range(d.shape[2])))),
		(1,2,0))

def load_sample_group_fmri():
	f = np.load(os.path.join(MAT_DIR, 'sample_group_fmri.npy'))
	import functools
	...
	return np.transpose(list(map(compose(bct.normalize,thresh_fun),
		(f[:,:,i] for i in range(f.shape[2])))),(1,2,0))
